...
void Archive::RequestArcPassword()
{
  if (!Cmd->Password.IsSet())
  {
    ...
    if (Cmd->Callback!=NULL)
    {
      ...
      *PasswordW=0;
      if (Cmd->Callback(UCM_NEEDPASSWORDW,Cmd->UserData,(uintptr_t)PasswordW,ASIZE(PasswordW))==-1)
        *PasswordW=0;
      if (*PasswordW==0)
      {
        char PasswordA[MAXPASSWORD];
        *PasswordA=0;
        if (Cmd->Callback(UCM_NEEDPASSWORD,Cmd->UserData,(uintptr_t)PasswordA,ASIZE(PasswordA))==-1)
          *PasswordA=0;

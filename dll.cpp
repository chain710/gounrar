...
void* PASCAL RAROpenArchive( RAROpenArchiveData *r)
{
...
void* PASCAL RAROpenArchiveEx( RAROpenArchiveDataEx *r)
{
  DataSet *Data=NULL;
  try
  {
...
int PASCAL RARReadHeader(void* hArcData, RARHeaderData *D)
{
  RARHeaderDataEx X;
  memset(&X,0,sizeof(X));
...
int PASCAL RARReadHeaderEx(void* hArcData, RARHeaderDataEx *D)
{
  DataSet *Data=(DataSet *)hArcData;
  try
  {
    ...
    D->BlockPos = Data->Arc.CurBlockPos;
    D->MtimeUnix = hd->mtime.GetUnixNS();
    D->AtimeUnix = hd->atime.GetUnixNS();
    D->CtimeUnix = hd->ctime.GetUnixNS();
    D->Type = Data->Arc.GetHeaderType();
...
int PASCAL ProcessFile(void* hArcData,int Operation,char *DestPath,char *DestName,wchar *DestPathW,wchar *DestNameW)
{
  DataSet *Data=(DataSet *)hArcData;
  try
  {
...
int PASCAL RARProcessFileW(void* hArcData,int Operation,wchar *DestPath,wchar *DestName)
{
...
{
    DataSet *Data=(DataSet *)hArcData;
    return Data->Arc.Seek(blockPos,SEEK_SET)? ERAR_SUCCESS: ERAR_ESEEK;
}

//int PASCAL RARGetLastError(char *Msg, size_t Len) {
//    wchar ErrMsg[1024] = {0};
//    if (!ErrHandler.GetSysErrMsg(ErrMsg, ASIZE(ErrMsg))) {
//        return -1;
//    }
//    if (!WideToChar(ErrMsg, Msg, Len)) {
//        return -2;
//    }
//    return 0;
//}
...
bool DllVolChange(RAROptions *Cmd,wchar *NextName,size_t NameSize)
{
  ...
  if (Cmd->Callback!=NULL)
  {
    ...
    if (Cmd->Callback(UCM_CHANGEVOLUMEW,Cmd->UserData,(uintptr_t)NextName,RAR_VOL_ASK)==-1)
      DllVolAborted=true;
    else
      ...
      else
      {
        ...
        if (Cmd->Callback(UCM_CHANGEVOLUME,Cmd->UserData,(uintptr_t)NextNameA,RAR_VOL_ASK)==-1)
          DllVolAborted=true;
        else
...
bool DllVolNotify(RAROptions *Cmd,wchar *NextName)
{
  ...
  if (Cmd->Callback!=NULL)
  {
    if (Cmd->Callback(UCM_CHANGEVOLUMEW,Cmd->UserData,(uintptr_t)NextName,RAR_VOL_NOTIFY)==-1)
      return false;
    if (Cmd->Callback(UCM_CHANGEVOLUME,Cmd->UserData,(uintptr_t)NextNameA,RAR_VOL_NOTIFY)==-1)
      return false;
  }
